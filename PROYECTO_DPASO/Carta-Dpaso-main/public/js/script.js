...
function updateCheckoutIdentityControlsVisibility(user = null) {
  ...
  const forOtherCheckbox = document.getElementById('checkout-for-other');
  const saveProfileCheckbox = document.getElementById('checkout-save-profile');
  ...
  if (saveProfileCheckbox) {
    saveProfileCheckbox.disabled = !loggedIn || Boolean(forOtherCheckbox?.checked);
    if (!loggedIn) saveProfileCheckbox.checked = false;
  }
}
...
function setupCartModalEvents() {
  ...
  bindScopedListener(scope, forOtherCheckbox, 'change', () => {
    ...
    if (forOtherCheckbox?.checked) {
      ...
      if (saveProfileCheckbox) {
        ...
        saveProfileCheckbox.disabled = true;
      }
      return;
    }
    if (saveProfileCheckbox) saveProfileCheckbox.disabled = false;
  ...
  bindScopedListener(scope, goCheckoutBtn, 'click', () => {
    ...
    clearFeedback();
    const saveProfileCheckbox = document.getElementById('checkout-save-profile');
    if (saveProfileCheckbox) saveProfileCheckbox.disabled = Boolean(forOtherCheckbox?.checked);
...
async function submitOrder(eventOrForm) {
  ...
  const saveProfileRequested = Boolean(sessionUser?.id)
    && Boolean(document.getElementById('checkout-save-profile')?.checked)
    && !orderForOther;
  ...
  let createdAt = null;
    ...
    if (rpcError) {
      ...
      if (finalMsg.includes('PHONE_ALREADY_LINKED') && orderForOther) {
    ...
    if (saveProfileRequested) {
      const profileSaved = await persistCheckoutProfileData({
        name: customerName,
        phone: telefonoLimpio,
        email: emailRaw || sessionUser?.email || ''
      });
      if (profileSaved) {
        showCartToast('Datos guardados en tu perfil', 2600);
      }
    }
  } catch (error) {
...
async function persistCheckoutProfileData({ name = '', phone = '', email = '' } = {}) {
  ...
  try {
    ...
    const first_name = parts.shift() || normalizedName;
    ...
    const { error: metadataError } = await withTimeout(
      supabaseClient.auth.updateUser({ data: metadataPayload }),
      12000,
      'No se pudo actualizar el perfil a tiempo.'
    );
    ...
    const { error: customerError } = await withTimeout(
      supabaseClient.rpc('upsert_my_customer_profile', {
        ...
        p_dni: null,
        p_email: normalizedEmail || user.email || null,
        p_avatar_path: null
      }),
      12000,
      'No se pudo sincronizar customers a tiempo.'
    );
    if (customerError) throw customerError;

    return true;
  } catch (error) {
    logSupabaseError('⚠️ No se pudo guardar datos de checkout en perfil:', error);
    return false;
  }
}
...
async function loadProfileOrderDetail(shortCode = '') {
  ...
  try {
    let data = null;
    const { data: privateData, error: privateError } = await withTimeout(
      supabaseClient.rpc('get_my_order_status', { p_short_code: code }),
      9000,
      'No se pudo cargar el detalle del pedido a tiempo.'
    );

    if (privateError) {
      const isMissingRpc = String(privateError?.message || '').includes('get_my_order_status')
        || String(privateError?.code || '') === 'PGRST202';
      if (!isMissingRpc) throw privateError;
