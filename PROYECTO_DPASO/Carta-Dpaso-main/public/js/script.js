...
function initInfiniteCarousel(row) {
  ...
  const jumpBy = (direction = 1) => {
    const first = originals[0];
    if (!enabled || !first) return;
    setPause(true);
    viewport.scrollLeft += direction * (first.getBoundingClientRect().width + 14);
    resetToLoop();
    setPause(false, true);
  };
  ...
  const recalc = () => {
    ...
    prev.style.display = enabled ? 'inline-flex' : 'none';
    next.style.display = enabled ? 'inline-flex' : 'none';
