...
function setAuthEmailValue(email = '', { force = false } = {}) {
  ...
  if (!force && emailInput.dataset.userEdited === '1') return;
  if (!force && document.activeElement === emailInput) return;
  if (!force && emailInput.value.trim()) return;
  emailInput.value = normalizeEmail(email);
}
...
function setAuthMode(mode = 'login') {
  ...
  if (authPasswordInput) authPasswordInput.setAttribute('autocomplete', normalized === 'register' ? 'new-password' : 'current-password');
  ...
  if (emailInput && normalized === 'login') emailInput.setAttribute('autocomplete', 'email');
...
function setupAuthEvents() {
  ...
  bindScopedListener(scope, authEmailInput, 'input', () => {
    if (authEmailInput) authEmailInput.dataset.userEdited = '1';
  }, {}, 'auth:email-edited');
  ...
  bindScopedListener(scope, accountOpenProfile, 'click', () => {
    closeAccountDropdown();
    openProfileModal();
  }, {}, 'auth:menu-profile');
  ...
  bindScopedListener(scope, document.getElementById('profile-form'), 'submit', async (event) => {
    ...
    try {
      ...
      const updateResult = await withTimeout(
        ...
        12000,
        'La actualización del perfil tardó demasiado.'
      );
