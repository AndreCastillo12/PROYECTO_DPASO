...
async function cargarMenu() {
  ...
  try {
    const platosBaseFields = 'id,nombre,descripcion,precio,imagen,categoria_id,orden,is_available,track_stock,stock';
    const platosEnhancedFields = `${platosBaseFields},promo_price,precio_promo,precio_oferta,price_promo,is_best_seller,mas_vendido,is_new,nuevo`;

    let platosData = [];
    let platosError = null;

    ({ data: platosData, error: platosError } = await supabaseClient
      .from('platos')
      .select(platosEnhancedFields)
      .order('orden', { ascending: true }));

    if (platosError) {
      console.warn('⚠️ Campos opcionales no disponibles en platos. Usando query base.', platosError?.message || platosError);
      ({ data: platosData, error: platosError } = await supabaseClient
        .from('platos')
        .select(platosBaseFields)
        .order('orden', { ascending: true }));
    }
