...
async function syncCustomerProfileToCustomers(payload = {}) {
  try {
    ...
    logSupabaseError('⚠️ No se pudo sincronizar customers (no bloqueante):', error);
  }
}
...
function applyAuthUi(user = null, customerProfile = null) {
  ...
  if (user) {
    ...
    setVal('profile-first-name', user.user_metadata?.first_name || profileFirst || first);
    setVal('profile-last-name', user.user_metadata?.last_name || profileLast || last);
...
function setupAuthEvents() {
  ...
  bindScopedListener(scope, document.getElementById('profile-form'), 'submit', async (event) => {
    ...
    try {
      ...
      await syncCustomerProfileToCustomers(customerPayload);

      const successMessage = authMetadataUpdated
        ? 'Perfil actualizado correctamente.'
        : 'Perfil guardado con cambios locales. Recarga si no ves todos los datos al instante.';
      ...
      try {
        ...
        logSupabaseError('⚠️ Perfil guardado, pero no se pudo refrescar UI inmediatamente:', refreshError);
      }
    } catch (error) {
