    ...
    categoriasData.forEach((cat) => {
      ...
      items.forEach((item) => {
        ...
        card.innerHTML = `
          ...
          ${soldOut ? `<div class="plato-unavailable" aria-live="polite">${availabilityMessage}</div>` : `<button type="button" class="add-cart-btn" data-item-id="${item.id}">+ Agregar</button>`}
        `;
...
async function loadProfileOrderDetail(shortCode = '') {
  ...
  try {
    ...
    const canCancel = canCustomerCancelOrder(data?.status);
    detail.innerHTML = `
      <div class="tracking-order-meta">
...
function setupAuthEvents() {
  ...
  bindScopedListener(scope, document.getElementById('profile-form'), 'submit', async (event) => {
    ...
    try {
      ...
      if (!customerProfileSynced && appRuntime.lastCustomerSyncError) {
        throw new Error(appRuntime.lastCustomerSyncError);
      }
