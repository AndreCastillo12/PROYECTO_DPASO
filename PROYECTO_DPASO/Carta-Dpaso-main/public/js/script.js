...
async function submitOrder(eventOrForm) {
  ...
  let createdAt = null;
    ...
    if (errorMessage.includes('PHONE_ALREADY_LINKED')) {
      ...
      showFeedback('No se pudo crear el pedido. Revisa tu conexión o intenta de nuevo. Si usas Google, intenta confirmar una vez más.', 'error');
    }
  } finally {
...
function validateRegisterForm({ firstName, lastName, phone, dni, email, password, confirmPassword }) {
  const errors = {};
  const nameRegex = /^[A-Za-zÁÉÍÓÚáéíóúÑñÜü' -]{2,}$/;

  if (!firstName) errors.firstName = 'Ingresa tus nombres.';
  else if (!nameRegex.test(firstName)) errors.firstName = 'Nombres inválidos.';

  if (!lastName) errors.lastName = 'Ingresa tus apellidos.';
  else if (!nameRegex.test(lastName)) errors.lastName = 'Apellidos inválidos.';
...
function setupAuthEvents() {
  ...
  const validateRecoveryEmail = () => {
    ...
    if (passwordSubmitBtn) passwordSubmitBtn.disabled = !ok || Boolean(appRuntime.recoveryBusy);
    return { email, ok };
  };
  ...
  bindScopedListener(scope, passwordForm, 'submit', async (event) => {
    ...
    const previousText = passwordSubmitBtn?.textContent || 'Enviar enlace';
    ...
    try {
      ...
      if (error) throw error;
      if (feedback) {
        feedback.textContent = 'Si el correo está registrado, te enviaremos un enlace para cambiar tu contraseña.';
      ...
      if (passwordSubmitBtn) {
        passwordSubmitBtn.textContent = previousText;
      }
      validateRecoveryEmail();
    }
  ...
  bindScopedListener(scope, authForm, 'submit', async (event) => {
    ...
    try {
      ...
      showAuthFeedback(String(error?.message || 'Ocurrió un error, intenta nuevamente.'), 'error');
    } finally {
      appRuntime.authBusy = false;
      if (submitBtn) {
        submitBtn.disabled = false;
        submitBtn.textContent = appRuntime.authMode === 'register' ? 'Crear cuenta' : 'Ingresar';
      }
    }
  }, {}, 'auth:submit');
  ...
  bindScopedListener(scope, document.getElementById('profile-form'), 'submit', async (event) => {
    ...
    if (phone && phone.length !== 9) {
      showProfileFeedback('El teléfono debe tener 9 dígitos.', 'error');
      return;
    }
    if (dni && dni.length !== 8) {
      showProfileFeedback('El DNI debe tener 8 dígitos.', 'error');
      return;
    }
