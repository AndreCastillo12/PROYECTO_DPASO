...
function getOrCreateListenerScope(scopeName) {
  const previous = appRuntime.listenerScopes.get(scopeName);
  if (previous) {
    previous.controller.abort();
    previous.actions?.forEach((key) => appRuntime.boundActionKeys.delete(key));
  }
...
function bindScopedListener(scope, target, type, handler, options = {}, actionKey = null) {
  ...
  const previousCount = Number(appRuntime.listenerStats[key] || 0);
  appRuntime.listenerStats[key] = previousCount + 1;
