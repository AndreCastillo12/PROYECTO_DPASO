...
begin
  ...
  if exists (
    ...
    alter table public.customers
      drop constraint customers_dni_check;
  end if;

  alter table public.customers
    add constraint customers_dni_check
    check (dni is null or btrim(dni) ~ '^[0-9]{8}$');
end $$;

-- Limpieza defensiva de DNI legado inválido antes de operar updates/inserts.
...
begin
  ...
  if v_customer_id is null then
    ...
    set
      ...
      dni = coalesce(v_dni, case when c.dni ~ '^[0-9]{8}$' then c.dni else null end),
      account_type = 'registered',
      updated_at = now()
