...
drop function if exists public.rpc_upsert_my_customer_profile(text, text);
create or replace function public.rpc_upsert_my_customer_profile(p_name text, p_phone text)
returns jsonb
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  if v_customer_id is null then
    ...
    values (v_name, v_phone, v_phone, v_uid, v_email, 'registered')
    returning id into v_customer_id;
  else
    update public.customers c
    set
...
revoke all on function public.rpc_upsert_my_customer_profile(text, text) from public;
grant execute on function public.rpc_upsert_my_customer_profile(text, text) to authenticated;
