...
alter table if exists public.cash_movements
  add column if not exists movement_source text not null default 'manual',
...
alter table if exists public.cash_movements
  add constraint cash_movements_source_check check (movement_source in ('manual', 'order_sale'));

create index if not exists cash_movements_source_idx on public.cash_movements(movement_source);
create unique index if not exists cash_movements_order_sale_uidx
...
-- ---------------------------------------------------------------------------
-- Trigger: registrar venta en efectivo en movimientos de caja
-- ---------------------------------------------------------------------------
...
begin
  ...
  if not (coalesce(new.paid, false) = true
      and lower(coalesce(new.estado, '')) = 'completed') then
    return new;
  end if;

  v_effective_time := coalesce(new.paid_at, now());

  select cs.id into v_session_id
  from public.cash_sessions cs
  where cs.status = 'open'
    and cs.opened_at <= v_effective_time
  ...
  values (
    v_session_id,
    'in',
    'Venta ' || upper(coalesce(nullif(new.payment_method, ''), 'UNKNOWN')) || ' pedido ' || v_order_code,
...
after update of paid, payment_method, estado on public.orders
for each row
...
create or replace function public.rpc_close_cash_session(session_id uuid, p_closing_amount numeric, p_notes text default null)
returns jsonb
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  where coalesce(o.paid_at, o.created_at) >= v_session.opened_at
    and coalesce(o.paid_at, o.created_at) <= v_closed_at
    and lower(coalesce(o.estado, '')) = 'completed'
  ...
  set
    ...
    difference = v_diff,
    notes = coalesce(nullif(trim(coalesce(p_notes, '')), ''), v_session.notes),
    status = 'closed'
