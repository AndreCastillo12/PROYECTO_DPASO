-- Sprint 44: bootstrap primer admin + limpieza UI usuarios

-- Permite reclamar rol admin solo una vez cuando no existen roles internos.
...
begin
  ...
  select count(*) into v_roles_count from public.admin_panel_user_roles;
  if coalesce(v_roles_count, 0) > 0 then
  ...
  -- Criterio bootstrap: usuario explícitamente admin en profile o correo corporativo.
  if not (v_profile_role = 'admin' or v_email like '%@dpaso.com') then
