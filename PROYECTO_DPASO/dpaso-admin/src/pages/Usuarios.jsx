...
export default function Usuarios() {
  ...
  const createInternalUser = async () => {
    ...
    if (!payload.password) {
      const { error: assignOnlyError } = await supabase.rpc("rpc_admin_set_user_role_by_email", {
      ...
      if (assignOnlyError) {
        ...
        showToast(buildFunctionsOfflineMessage(errMsg(assignOnlyError, "No se pudo asignar rol interno por email")), "error");
        setBusy(false);
        return;
      }
      ...
      await loadUsers();
      setBusy(false);
      showToast("Rol interno asignado a usuario existente en Auth", "success");
      return;
    }

    // 1) intento función nueva
    ...
    // 3) fallback: si ya existe en Auth, asignar rol por email
    const { error: assignError } = await supabase.rpc("rpc_admin_set_user_role_by_email", {
      p_email: payload.email.toLowerCase(),
      p_role: payload.role,
    });
    ...
    if (!assignError) {
      ...
      await loadUsers();
      setBusy(false);
      showToast("Edge Function no disponible. Se asignó rol interno a usuario Auth existente.", "success");
      return;
    }

    const invokeError = await getInvokeError(primary.error || legacy.error, "No se pudo crear usuario interno");
    const message = buildFunctionsOfflineMessage(invokeError);
    setFunctionsAvailable(false);
    showToast(assignError ? message : invokeError, "error");
    setBusy(false);
  };
  ...
  return (
    <div style={{ display: "grid", gap: 14 }}>
      <Toast toast={toast} />
      <h2 style={{ margin: 0 }}>Usuarios trabajadores</h2>

      {!functionsAvailable ? (
        <p style={{ margin: 0, padding: 10, borderRadius: 8, background: "#fff7ed", color: "#9a3412" }}>
          Edge Functions de admin no disponibles. Puedes gestionar roles de usuarios ya existentes en Auth; para crear/eliminar/reset completo despliega functions.
        </p>
      ) : null}

      <section style={card}>
        <h3 style={{ margin: 0 }}>Usuarios trabajadores (CRUD)</h3>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 8 }}>
          <input style={inputStyle} value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Correo trabajador" />
          <input style={inputStyle} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Contraseña (opcional si ya existe en Auth)" type="password" />
          <select style={{ ...inputStyle, minWidth: 140 }} value={newRole} onChange={(e) => setNewRole(e.target.value)}>
            {ROLES.map((r) => <option key={r} value={r}>{r}</option>)}
          </select>
          <button type="button" style={btnPrimary} disabled={busy || currentRole !== "admin"} onClick={createInternalUser}>Crear / Asignar rol</button>
        </div>

        <div style={{ display: "grid", gap: 8, marginTop: 10 }}>
          {managedUsers.length === 0 ? <p style={{ color: "#6b7280" }}>No hay otros usuarios internos.</p> : managedUsers.map((u) => (
            <div key={u.user_id} style={rowStyle}>
              <div>
                <strong>{u.nombre ? `${u.nombre} ${u.apellidos || ""}`.trim() : u.email}</strong>
                <small style={{ display: "block", color: "#6b7280" }}>{u.email} · rol: {u.role}</small>
              </div>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                <button type="button" style={btnGhost} disabled={busy || currentRole !== "admin"} onClick={() => changeRole(u.user_id, u.role, u.email)}>Editar rol</button>
