...
export default function Dashboard() {
  ...
  const chartMax = useMemo(() => {
    ...
    return Math.max(...chartData.map((item) => Math.max(item.sales, item.goal)), 1);
  }, [chartData]);
  ...
  return (
    <div style={pageWrap}>
      ...
      <section style={cardStyle}>
        <div style={sectionHeader}>
          <h3 style={sectionTitle}>Top ventas · Metas vs ventas</h3>
          ...
          <div style={chartWrap}>
            {chartData.map((item) => (
              <div key={item.day} style={chartCol}>
                <div style={chartSlot}>
                  <div style={{ ...chartGoalLine, bottom: `${(item.goal / chartMax) * 100}%` }} />
