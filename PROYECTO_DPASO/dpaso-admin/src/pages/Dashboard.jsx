...
export default function Dashboard() {
  ...
  const donutData = useMemo(() => {
    const cancelled = latestOrders.filter((o) => o.estado === "cancelled").length;
    const delivered = latestOrders.filter((o) => ["delivered", "completed"].includes(o.estado)).length;
    const pending = Math.max(latestOrders.length - delivered - cancelled, 0);
    const total = Math.max(latestOrders.length, 1);
    return [
      { label: "Entregados", pct: Math.round((delivered / total) * 100), color: "#2ab079" },
      { label: "Pendientes", pct: Math.round((pending / total) * 100), color: "#6eb7ff" },
      { label: "Cancelados", pct: Math.round((cancelled / total) * 100), color: "#ff6f78" },
    ];
  }, [latestOrders]);
    ...
    <div className="sedap-dashboard v2">
      ...
      <section className="sedap-analytics-grid">
        <article className="sedap-card pie-card">
          <div className="sedap-card-head"><h4>Pie chart</h4></div>
