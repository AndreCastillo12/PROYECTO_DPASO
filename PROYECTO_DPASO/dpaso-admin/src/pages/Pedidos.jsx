...
export default function Pedidos() {
  ...
  useEffect(() => {
    ...
    const found = orders.find((o) => o.id === orderIdFromQuery);
    if (!found) return;
  ...
  const getOrderCashMovementState = useCallback(async (orderId) => {
    if (!orderId) return { hasMovement: false, inClosedSession: false };

    const { data, error } = await supabase
      .from("cash_movements")
      .select("id, cash_session_id, cash_sessions(status)")
      .eq("order_id", orderId)
      .eq("movement_source", "order_sale")
      .order("created_at", { ascending: false })
      .limit(1)
      .maybeSingle();

    if (error) {
      return { hasMovement: false, inClosedSession: false, error };
    }

    return {
      hasMovement: Boolean(data?.id),
      inClosedSession: data?.cash_sessions?.status === "closed",
    };
  }, []);

  const hasOpenCashSession = useCallback(async () => {
    const { data, error } = await supabase
      .from("cash_sessions")
      .select("id")
      .eq("status", "open")
    ...
    return { open: Boolean(data?.id), error: null };
  }, []);
  ...
  const onChangeStatus = async (newStatus) => {
    ...
    const movementState = await getOrderCashMovementState(orderId);
    if (movementState.error) {
      showToast("No se pudo validar caja para este pedido", "error");
      return;
    }
    if (movementState.inClosedSession) {
      showToast("Este pedido ya fue registrado en una caja cerrada y quedó bloqueado para edición.", "error");
      return;
    }

    if (newStatus === "completed" && Boolean(selectedOrder.paid) && !movementState.hasMovement) {
      const cashState = await hasOpenCashSession();
      if (cashState.error || !cashState.open) {
        showToast("No puedes completar un pedido pagado sin caja abierta.", "error");
        return;
      }
    }
  ...
  const onSavePayment = async () => {
    ...
    const movementState = await getOrderCashMovementState(selectedOrder.id);
    if (movementState.error) {
      showToast("No se pudo validar caja para este pedido", "error");
      return;
    }
    ...
    if (isPaid && selectedOrder.estado === "completed" && !movementState.hasMovement) {
      const cashState = await hasOpenCashSession();
      if (cashState.error || !cashState.open) {
        showToast("No puedes marcar pagado/completado sin caja abierta.", "error");
        return;
      }
    }
        ...
        <aside style={detailCard}>
          {!selectedOrder ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div style={miniCard}>
                <h4 style={miniCardTitle}>Operación</h4>
              ...
              <div style={miniCard}>
                <h4 style={miniCardTitle}>Cobro</h4>
                <p style={labelLine}><strong>Método actual:</strong> {humanPayment(selectedOrder.payment_method)}</p>
