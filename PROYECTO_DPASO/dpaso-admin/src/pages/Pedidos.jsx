...
function getStatusFlow(order) {
  if (isSalonOrder(order)) return PICKUP_FLOW;
...
function orderAccountType(order) {
  return order?.user_id ? "Registrado" : "Invitado";
}

function orderDeliverySnapshot(order) {
  const name = order?.delivery_name || order?.nombre_cliente || "-";
  const phone = order?.delivery_phone || order?.telefono || "-";
  const address = isSalonOrder(order) ? `Mesa ${order?.table_number || "-"}` : (order?.delivery_address || order?.direccion || order?.referencia || "-");
...
export default function Pedidos() {
  ...
  const filteredOrders = useMemo(() => {
    return orders.filter((order) => {
      if (isSalonOrder(order)) return false;
        ...
        <table>
          ...
          <tbody>
            {filteredOrders.map((order, index) => {
              ...
              const accountType = orderAccountType(order);
              const deliverySnapshot = orderDeliverySnapshot(order);
              const shouldOpenUp = index >= filteredOrders.length - 3;
              return (
                <tr key={order.id}>
                  <td>#{shortCode(order)}</td>
                  <td>{new Date(order.created_at).toLocaleString()}</td>
                  <td><span className={`account-badge ${order.user_id ? "registered" : "guest"}`}>{accountType}</span></td>
                  <td>{order.nombre_cliente || "Cliente"}</td>
                  <td>
                    <div className="delivery-snapshot">
                      <strong>{deliverySnapshot.name}</strong>
                      <small>{deliverySnapshot.phone}</small>
                      <small>{deliverySnapshot.address}</small>
                    </div>
                  </td>
                  ...
                  <td>
                    <div className="row-actions" ref={openMenuOrderId === order.id ? menuHostRef : null}>
                      ...
                      {openMenuOrderId === order.id ? (
                        <div className={`row-menu ${shouldOpenUp ? "row-menu-up" : ""}`}>
