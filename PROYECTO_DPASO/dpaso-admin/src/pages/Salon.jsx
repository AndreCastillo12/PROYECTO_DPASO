...
export default function Salon() {
  ...
  const openTicketByTable = useMemo(() => {
    ...
    tickets.forEach((t) => {
      ...
      if (isReleased || !t.table_id) return;
  ...
  if (!canAccess("salon")) return <p>No tienes permisos para Local.</p>;
  if (loading) return <p>Cargando módulo Local...</p>;

  return (
    <div style={{ display: "grid", gap: 14 }}>
      <Toast toast={toast} />
      <h2 style={{ margin: 0 }}>Local POS</h2>
        ...
        {activeTables.length === 0 ? <p>No hay mesas activas</p> : (
          <>
            ...
            <div style={{ marginTop: 10, display: "flex", gap: 8, flexWrap: "wrap" }}>
              ...
              {selectedTicket ? <small>Ticket: <strong>{selectedTicket.id.slice(0, 8).toUpperCase()}</strong> · {selectedTicket.status} · {selectedTicket.payment_status}</small> : null}
            </div>
          </>
        )}
      </section>
