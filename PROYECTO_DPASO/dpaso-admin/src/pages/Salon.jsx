...
function mapErrorMessage(message, fallback) {
  ...
  if (m.includes("orders_modalidad_check")) return "Falta actualizar constraint de modalidad. Ejecuta sprint35.";
  if (m.includes("table_number") || m.includes("table_ticket_open")) return "Faltan columnas de Salón en orders (sprint34/sprint35).";
  if (m.includes("relation") && m.includes("restaurant_tables") && m.includes("does not exist")) return "No existe la tabla restaurant_tables en este proyecto. Ejecuta sprint35.";
  if (m.includes("permission") && m.includes("restaurant_tables")) return "No tienes permisos para leer/gestionar mesas (restaurant_tables).";
  if (m.includes("schema cache") && (m.includes("table_number") || m.includes("table_ticket_open") || m.includes("restaurant_tables"))) {
    return "Tu proyecto aún muestra cache viejo de Supabase. Reintenta en 1-2 min y recarga.";
  }
...
export default function Salon() {
  ...
  async function loadSalonData() {
    ...
    const [tableResp, ordersResp, platosResp] = await Promise.all([
    ...
    let ordersData = ordersResp.data;
    let ordersError = ordersResp.error;

    if (ordersError && (String(ordersError.message || "").toLowerCase().includes("table_ticket_open") || String(ordersError.message || "").toLowerCase().includes("table_number"))) {
      const fallback = await supabase
        .from("orders")
        .select("id,short_code,modalidad,estado,total,paid,payment_method,created_at")
        .eq("modalidad", "salon")
        .order("created_at", { ascending: false })
        .limit(300);
      ordersData = (fallback.data || []).map((o) => ({ ...o, table_number: null, table_ticket_open: false }));
      ordersError = fallback.error;
    }

    if (platosResp.error) {
      showToast(mapErrorMessage(platosResp.error.message, "No se pudo cargar la carta de Salón"), "error");
      setLoading(false);
      return;
    }

    if (ordersError) {
      showToast(mapErrorMessage(ordersError.message, "No se pudo cargar tickets de Salón"), "error");
      setLoading(false);
      return;
    }
    ...
    if (tableResp.error) {
      showToast(mapErrorMessage(tableResp.error.message, "No se pudieron cargar mesas administrables"), "warning");
    }

    const derivedTables = tableRows.length > 0
      ? tableRows
      : Array.from(new Set(orderRows.map((o) => String(o.table_number || "").trim()).filter(Boolean))).map((name) => ({ id: `legacy-${name}`, table_name: name, active: true }));

    setTables(derivedTables);
    setOrders(orderRows);
    setPlatos(platosResp.data || []);

    if (!selectedTable && derivedTables.length > 0) {
      setSelectedTable(derivedTables[0]);
    }
