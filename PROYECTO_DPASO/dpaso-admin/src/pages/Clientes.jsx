...
export default function Clientes() {
  ...
  const loadClients = useCallback(async ({ silent = false } = {}) => {
    ...
    let query = supabase.from("customers").select("id,name,phone,total_orders,total_spent,last_order_at,created_at").limit(300);
        ...
        <article className="clientes-table-card">
          {loading ? <p>Cargando clientes...</p> : refreshing ? <p>Actualizando resultados...</p> : clients.length === 0 ? <p>No hay clientes con los filtros actuales.</p> : (
            <div className="table-scroll">
              <table>
                <thead>
                  <tr><th>Nombre</th><th>Teléfono</th><th>Pedidos</th><th>Total gastado</th><th>Última compra</th></tr>
                </thead>
                <tbody>
                  {clients.map((c) => (
